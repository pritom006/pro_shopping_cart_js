...
function updateCartContainer(e) {
    ...
    let productContainer = addToCartBtn.parentElement;
    let productTitle = productContainer.getElementsByClassName('productTitle')[0].innerText;
    let productPrice = productContainer.getElementsByClassName('productPrice')[0].innerText;
    productimg = productContainer.getElementsByClassName('img')[0].src;

    let cartTitles = cartContainer.getElementsByClassName('cartTitle');
...
function AddRowInCart(productTitle, productPrice, productimg) {
    ...
    for (var i = 0; i < cartPrice.length; i++) {
        let priceFormatedNumb = cartPrice[i].innerText.replace('$', '').replace(',', '');
